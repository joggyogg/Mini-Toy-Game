...
public static class TerrainWFCGenerator
{
    ...
    public static void Generate(TerrainGridAuthoring grid, TerrainWFCConfig config, Terrain[] terrains, List<GradientLine> gradientLines)
    {
        ...
        if (terrains != null)
        {
            // Only apply the expensive heightmap bake to terrains whose footprint
            // actually overlaps tiles with non-zero corners. Terrains fully outside
            // the non-zero region are unchanged (already flat) and can be skipped,
            // saving the costly SetHeights + physics/LOD rebuild per chunk.
            Terrain[] toUpdate = FilterTerrainsOverlappingNonZeroArea(grid, terrains, heights, w, h);
            foreach (Terrain t in toUpdate)
                grid.ApplyToTerrain(t);
            StitchTerrainNeighbors(toUpdate);

            var splatMapper = grid.GetComponent<TerrainSplatMapper>();
            if (splatMapper != null)
                foreach (Terrain t in toUpdate)
                    splatMapper.PaintSplatmaps(t);
        }
    }
    ...
    public static void Clear(TerrainGridAuthoring grid, Terrain[] terrains)
    {
        ...
        if (terrains != null)
        {
            Terrain[] overlapping = FilterOverlappingTerrains(grid, terrains);
            foreach (Terrain t in overlapping)
                grid.ApplyToTerrain(t);
            StitchTerrainNeighbors(overlapping);
    ...
    // ── Terrain overlap filter ───────────────────────────────────────────────────

    /// <summary>
    /// Returns only terrains whose XZ bounds overlap the grid's world footprint.
    /// Prevents processing hundreds of non-overlapping chunks (each would trigger
    /// an expensive SetHeights + Flush for no reason).
    /// </summary>
    private static Terrain[] FilterOverlappingTerrains(TerrainGridAuthoring grid, Terrain[] terrains)
    {
        if (terrains == null || terrains.Length == 0) return System.Array.Empty<Terrain>();

        Vector3 gridPos = grid.transform.position;
        Vector3 gridRight = grid.transform.right;
        Vector3 gridForward = grid.transform.forward;
        Vector3 origin = grid.GridOriginLocalOffset;
        Vector2Int fullSize = grid.FullTileGridSize;

        // Grid world-space AABB (axis-aligned approximation — exact for unrotated grids).
        float gridWorldWidth = fullSize.x;  // FullTileWorldSize = 1
        float gridWorldDepth = fullSize.y;
        Vector3 gridMin = gridPos + gridRight * origin.x + gridForward * origin.z;
        Vector3 gridMax = gridMin + gridRight * gridWorldWidth + gridForward * gridWorldDepth;

        // Build an axis-aligned bounding box covering the grid footprint.
        float gMinX = Mathf.Min(gridMin.x, gridMax.x);
        float gMaxX = Mathf.Max(gridMin.x, gridMax.x);
        float gMinZ = Mathf.Min(gridMin.z, gridMax.z);
        float gMaxZ = Mathf.Max(gridMin.z, gridMax.z);

        var result = new System.Collections.Generic.List<Terrain>(terrains.Length);
        foreach (Terrain t in terrains)
        {
            if (t == null || t.terrainData == null) continue;
            Vector3 tPos = t.transform.position;
            ...
            float tMinX = tPos.x;
            float tMaxX = tPos.x + tSize.x;
            float tMinZ = tPos.z;
            float tMaxZ = tPos.z + tSize.z;

            // AABB overlap test in XZ.
            if (tMaxX <= gMinX || tMinX >= gMaxX || tMaxZ <= gMinZ || tMinZ >= gMaxZ)
                continue;
    ...
    /// <summary>
    /// Returns only terrains whose XZ bounds overlap the world-space bounding box of
    /// corners with non-zero heights. Terrains fully outside this area already have
    /// flat heightmaps and can skip the expensive ApplyToTerrain + SetHeights pipeline.
    /// </summary>
    private static Terrain[] FilterTerrainsOverlappingNonZeroArea(
        TerrainGridAuthoring grid, Terrain[] terrains, int[,] heights, int w, int h)
    {
        if (terrains == null || terrains.Length == 0) return System.Array.Empty<Terrain>();

        // Find the tile-space AABB of all non-zero corners.
        int minCx = w, maxCx = -1, minCz = h, maxCz = -1;
        for (int cz = 0; cz < h; cz++)
        {
            for (int cx = 0; cx < w; cx++)
            {
        ...
        // No non-zero corners → nothing changed; ApplyToTerrain's anyChanged guard
        // will skip SetHeights, but we can avoid even the pixel loop entirely.
        if (maxCx < 0) return System.Array.Empty<Terrain>();

        // Convert tile-space AABB to world-space (FullTileWorldSize = 1).
        // Expand by 1 tile to account for bilinear interpolation bleeding.
        Vector3 gridPos = grid.transform.position;
        Vector3 gridRight = grid.transform.right;
        Vector3 gridForward = grid.transform.forward;
        Vector3 origin = grid.GridOriginLocalOffset;

        float tileMinX = Mathf.Max(0, minCx - 1);
        float tileMaxX = maxCx + 1;
        float tileMinZ = Mathf.Max(0, minCz - 1);
        float tileMaxZ = maxCz + 1;

        Vector3 wMin = gridPos + gridRight * (origin.x + tileMinX) + gridForward * (origin.z + tileMinZ);
        Vector3 wMax = gridPos + gridRight * (origin.x + tileMaxX) + gridForward * (origin.z + tileMaxZ);

        float gMinX = Mathf.Min(wMin.x, wMax.x);
        float gMaxX = Mathf.Max(wMin.x, wMax.x);
        float gMinZ = Mathf.Min(wMin.z, wMax.z);
        float gMaxZ = Mathf.Max(wMin.z, wMax.z);
        ...
        foreach (Terrain t in terrains)
        {
            ...
            if (tPos.x + tSize.x <= gMinX || tPos.x >= gMaxX ||
                tPos.z + tSize.z <= gMinZ || tPos.z >= gMaxZ)
                continue;
    ...
    public static void StitchTerrainNeighbors(Terrain[] terrains)
    {
        ...
        // Note: Flush() is intentionally omitted. SetHeights() already commits
        // the heightmap data and SetNeighbors() takes effect lazily during
        // rendering. Calling Flush() on every chunk forces an immediate
        // LOD + physics rebuild per terrain and causes long editor hangs.
    }
}
