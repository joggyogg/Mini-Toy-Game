...
public class TerrainEditController : MonoBehaviour
{
    ...
    private void ApplyAndRefresh(RectInt dirtyTiles)
    {
        ...
        foreach (Terrain t in childTerrains)
        {
            if (t != null)
            {
                terrainGrid.ApplyToTerrainPartial(t, dirtyTiles);
                if (splatMapper != null) splatMapper.PaintSplatmapsPartial(t, dirtyTiles, terrainGrid);
            }
        }
        TerrainWFCGenerator.StitchTerrainNeighbors(childTerrains);
    }
}
