...
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Input.cs.hlsl"
#endif
...
{
    float3 wpos = GetAbsolutePositionWS(TransformObjectToWorld(vertex.xyz)).xyz;    //mul(unity_ObjectToWorld,vertex).xyz;
...
float4 UnityDistanceBasedTess (float4 v0, float4 v1, float4 v2, float minDist, float maxDist, float3 tess)
{
    float3 f;
    f.x = UnityCalcDistanceTessFactor (v0,minDist,maxDist,tess.x);
    f.y = UnityCalcDistanceTessFactor (v1,minDist,maxDist,tess.y);
    f.z = UnityCalcDistanceTessFactor (v2,minDist,maxDist,tess.z);
...
float4 UnityEdgeLengthBasedTess (float4 v0, float4 v1, float4 v2, float3 edgeLength)
{
    float3 pos0 = GetAbsolutePositionWS(TransformObjectToWorld(v0.xyz)).xyz;    //mul(unity_ObjectToWorld,v0).xyz;
    float3 pos1 = GetAbsolutePositionWS(TransformObjectToWorld(v1.xyz)).xyz;    //mul(unity_ObjectToWorld,v1).xyz;
    float3 pos2 = GetAbsolutePositionWS(TransformObjectToWorld(v2.xyz)).xyz;    //mul(unity_ObjectToWorld,v2).xyz;
    float4 tess;
    tess.x = UnityCalcEdgeTessFactor (pos1, pos2, edgeLength.x);
    tess.y = UnityCalcEdgeTessFactor (pos2, pos0, edgeLength.y);
    tess.z = UnityCalcEdgeTessFactor (pos0, pos1, edgeLength.z);
